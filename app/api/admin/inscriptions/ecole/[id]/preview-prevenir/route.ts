...
import { buildValidationEcoleEmail } from "@/lib/email/send";
import type { InscriptionEcoleRow } from "@/lib/types/db";

export const runtime = "nodejs";

/** Aperçu (HTML) de l'email « Prévenir » d'une inscription École, SANS envoi. */
export async function GET(
  ...
  const row = data as InscriptionEcoleRow;

  const { subject, html } = await buildValidationEcoleEmail(row);

  // Bandeau d'aperçu (destinataire + objet) au-dessus du rendu réel de l'email.
